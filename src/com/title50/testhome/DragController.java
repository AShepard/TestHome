...
public class DragController {
    ...
    //TODO private static final int VIBRATE_DURATION = 35;

    private static final boolean PROFILE_DRAWING_DURING_DRAG = false;

    private Context mContext;
    //TODO private Vibrator mVibrator;
     ...
     */
    public void startDrag(ImageCell imagecell, DragSource source, Object dragInfo, int dragAction) {
        // Start dragging, but only if the source has something to drag.
        boolean doDrag = source.allowDrag ();
        if (!doDrag) return;

        mOriginator = imagecell;

        Bitmap b = getViewBitmap(imagecell);
        ...
        imagecell.getLocationOnScreen(loc);
        int screenX = loc[0];
        int screenY = loc[1];

        startDrag(imagecell, b, screenX, screenY, 0, 0, b.getWidth(), b.getHeight(),
     ...
     *        {@link #DRAG_ACTION_COPY}
     */
    public void startDrag(ImageCell imagecell, Bitmap b, int screenX, int screenY,
        ...
        ApplicationInfo app = imagecell.getAppInfo();

        DragView dragView = mDragView = new DragView(mContext, app, b, registrationX, registrationY,
